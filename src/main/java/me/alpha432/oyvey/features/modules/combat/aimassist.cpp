...
void modules::Spinnybot::run(::cache& cache)
{
	if (sword_only)
	{
	...
	Spinnybot::intersection intersection = find_intersection(camera_pos, player_angles.get_vector(), box_min, box_max);
	...
	Spinnybot::intersection intersection2 = find_intersection(camera_pos, (player_angles + maths::angles{ float((delta.yaw > 0.0f ? 1.f : -1.f) * (std::abs(delta.yaw) + 0.1)), 0.f }).get_vector(), box_min, box_max);
...
void modules::Spinnybot::on_disable(::cache& cache)
{
	locked_target.clear_ref();
}

void modules::Spinnybot::render_options()
{
...
modules::aimassist::intersection modules::Spinnybot::find_intersection(maths::vector3d ray_start, maths::vector3d ray_dir, maths::vector3d box_min, maths::vector3d box_max)
{
